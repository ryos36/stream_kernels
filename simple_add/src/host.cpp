...
output.
    ...
    2) Increment
        This Kernel reads stream input, simple_add the value and streams to
output.
...
int main(int argc, char** argv) {
    ...
    cl::CommandQueue q;
    cl::Context context;
    cl::Kernel krnl_simple_add, krnl_mem_read, krnl_mem_write;
    ...
    for (size_t i = 0; i < data_size; i++) {
        source_input[i] = i & 0xFF;
        source_sw_results[i] = (i + 1) & 0xFF;
    }
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_simple_add = cl::Kernel(program, "simple_add", &err));
